...
import url_helpers#creating new url from dashboard outputs
...
COLS = {
    "storage": dict(
        ...
        "(range 0.08 – 11 TW)",
    ),#converting fractional value to numbers
    "curtailment": dict( #any action that reduces the amount of electricity generated to maintain the balance between supply and demand
        label="Curtailment",
        col="Curtailment",
    ...
    "biofuel": dict( #renewable energy produced by new/used vegetable oils and animal fats
    ...
    children=[
        dbc.NavItem(
            dbc.NavLink(
                ...
                href="https://doi.org/10.1016/j.joule.2022.05.009",#learn more and go to link
            )
        ),
        dbc.NavItem(
            dbc.NavLink(
...
def row_label(params, label, id, col, help_text, default_marks=False):
    if default_marks:
        kwargs = {}#pass variable length of keyword arguments to the function
    else:
        kwargs = {"marks": {0: "", 0.2: "", 0.4: "", 0.6: "", 0.8: "", 1: ""}}

    min_, max_ = df_spores[col].min(), df_spores[col].max()#max and mins of dataframe w CSV data
    return dbc.Row( #labels and rows for the program
        [
            dbc.Col(
                [
                    dbc.Label(
                    ...
                    dbc.Popover(#adding components to the bootstrap dashboard
            ...
            dbc.Col(
                url_helpers.apply_default_value(params)(dcc.RangeSlider)(
                    min=0,#min max values from the CSV file
                    max=1,
                    value=[min_, max_],
                    id=id,
                    className="slider",
...
def controls(params):#putting all of the dashboard elements togather & calling them?
    return html.Div(
        [
            dbc.Row(
                dbc.Col(
            ...
            row_label_from_id(params=params, id_="storage"),#different columns from the CSV
...
HELP_TEXT = [ #i am curious where this help text is shown in the program
    html.P(
...
def page_layout(params=None):
    ...
    results = (
        ...
        dbc.Tabs(#creating tabbed sections in dashboard
            [
                dbc.Tab(
                    html.Div(
                        [
